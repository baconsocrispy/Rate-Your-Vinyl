...
def searchKeywords(request):                                               # this function is to pass in the predefined {{ rvtdetails.google_keywords }} from REvitFuncions_RvtDetails.html
    if request.method == "GET":                                                 # and get JSON to return the search results.
        get_searchText = request.GET.get("Q")

        #Story6 API pt1, Step3: Connect to the API and write a basic JSON response (either to a txt file or the terminal)
        url = "https://google-search3.p.rapidapi.com/api/v1/crawl/q={}central=100".format(get_searchText)
        ...
        print(response.text)                                                # default items generated by Google Search API GET CRAWL: https://rapidapi.com/apigeek/api/google-search3/
                                                                                # after clicking search from broswer, terminal will generate huge amount of JSON data.

        return render(request, 'RevitFunctions/RevitFunctions_home.html')   # return to home page after completed, or it will be stuck in this page.
